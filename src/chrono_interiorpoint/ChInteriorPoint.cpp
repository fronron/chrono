...
namespace chrono
{
	...
	double ChInteriorPoint::Solve(ChSystemDescriptor& sysd)
	{
        ...
        rhs.c.MatrScale(-1); // adapt to InteriorPoint convention //TODO: check sign
        ...
        GetLog() << "x: " << var.x << "\n";
        GetLog() << "y: " << var.y << "\n";
        GetLog() << "lambda: " << var.lam << "\n";
        GetLog() << "ResidualNNorm primal: " << res.rp.NormTwo()/m << "\n";
        GetLog() << "ResidualNNorm dual: " << res.rd.NormTwo()/n << "\n";
        GetLog() << "Complementarity measure: " << var.y.MatrDot(var.y, var.lam) << "\n";
        GetLog() << "\n";
#endif
        ...
        return std::max({ res_nnorm.rp_nnorm, res_nnorm.rd_nnorm, res.mu });
	}
    ...
    ChInteriorPoint::IPresidual_nnorm_t& ChInteriorPoint::iterate()
	{
        ...
        var_pred.x.Resize(n,1);
        var_pred.y.Resize(m,1);
        var_pred.lam.Resize(m,1);
        Dvar_pred.x.Resize(n, 1);
        Dvar_pred.y.Resize(m,1);
        Dvar_pred.lam.Resize(m,1);

        /*** find directions ***/
        // update y/lambda diagonal submatrix
        if (ADD_COMPLIANCE)
            for (auto diag_sel = 0; diag_sel < m; diag_sel++)
                BigMat.SetElement(n + diag_sel, n + diag_sel, var.y.GetElement(diag_sel, 0) / var.lam.GetElement(diag_sel, 0) + E.GetElement(diag_sel, diag_sel));
        else
            for (auto diag_sel = 0; diag_sel < m; diag_sel++)
        ...
        /*** make the prediction step ***/
		  var_pred.y = Dvar.y;     var_pred.y.MatrScale(alfa_pred_prim);   var_pred.y += var.y;
		var_pred.lam = Dvar.lam; var_pred.lam.MatrScale(alfa_pred_dual); var_pred.lam += var.lam;
        ...
        //GetLog() << "Dvar.x: " << Dvar_pred.x << "\n";
        //GetLog() << "Dvar.y: " << Dvar_pred.y << "\n";
        //GetLog() << "Dvar.lambda: " << Dvar_pred.lam << "\n";
        //GetLog() << "\n";
        GetLog() << "Step length (prediction): P: " << alfa_pred_prim << " | D: " << alfa_pred_dual << "\n";
        //GetLog() << "\n";
        //GetLog() << "\n";
        //GetLog() << "Values (prediction):\n";
        //GetLog() << "var_pred.y: " << var_pred.y << "\n";
        //GetLog() << "lambda_pred: " << var_pred.lam << "\n";
        ...
        if (ONLY_PREDICT)
        {
            ...
            res_nnorm.rp_nnorm = res.rp.NormTwo() / m;
            res_nnorm.rd_nnorm = res.rd.NormTwo() / n;


#ifdef DEBUG_MODE
            GetLog() << "InteriorPoint Results onlvar_pred.y\n";
            GetLog() << "var_pred.x: " << var_pred.x << "\n";
            GetLog() << "ResidualNorm primal: " << res.rp.NormTwo() << "\n";
            GetLog() << "ResidualNorm dual: " << res.rd.NormTwo() << "\n";
            GetLog() << "\n";
#endif
            return res_nnorm;
		}
        ...
        //GetLog() << "Dvar.x: " << Dvar.x << "\n";
        //GetLog() << "Dvar.y: " << Dvar.y << "\n";
        //GetLog() << "Dvar.lambda: " << Dvar.lam << "\n";
        //GetLog() << "\n";
        ...
        GetLog() << "ResidualNNorm primal: " << res_nnorm.rp_nnorm << "\n";
        GetLog() << "ResidualNNorm dual: " << res_nnorm.rd_nnorm << "\n";
        GetLog() << "\n";
#endif
        ...
        if (history_file.is_open())
            history_file << std::endl << solver_call << ", " << iteration_count << ", " << res_nnorm.rp_nnorm << ", " << res_nnorm.rd_nnorm << ", " << res.mu << ", " << evaluate_objective_function();
	...
	// Solve the KKT system in different modes: 'res.rp', 'res.rd', 'res.mu', 'x', 'y', 'lam' must be updated before calling this function
	...
	void ChInteriorPoint::KKTsolve(double sigma, bool apply_correction)
	{
		...
		case IP_KKT_SOLUTION_METHOD::STANDARD:
			...
			for (auto diag_sel = 0; diag_sel < m; diag_sel++)
			{
				BigMat.SetElement(n + m + diag_sel, n + diag_sel, var.lam.GetElement(diag_sel, 0)); // write lambda diagonal submatrix
				BigMat.SetElement(n + m + diag_sel, n + m + diag_sel, var.y.GetElement(diag_sel, 0)); // write y diagonal submatrix
		...
		case IP_KKT_SOLUTION_METHOD::AUGMENTED:
			...
			for (auto row_sel = 0; row_sel < m; row_sel++)
				rhs_sol.SetElement(row_sel + n, 0, -res.rp(row_sel, 0) - var.y(row_sel, 0) + (apply_correction ? sigma*res.mu - Dvar.lam(row_sel,0)*Dvar.y(row_sel, 0) : sigma*res.mu) / var.lam(row_sel, 0));
		...
		case IP_KKT_SOLUTION_METHOD::NORMAL:
			...
			for (auto row_sel = 0; row_sel < n; row_sel++)
			{
				for (auto col_sel = 0; col_sel < n; col_sel++)
				{
					...
					{
						temp += var.lam(el_sel, 0) / var.y(el_sel, 0) * SmallMat.GetElement(el_sel, row_sel) * SmallMat.GetElement(el_sel, col_sel);
						if (temp != 0)
    ...
    ChMatrix<>& ChInteriorPoint::adapt_to_Chrono(ChMatrix<>& solution_vect) const
	{
		...
		{
			for (auto row_sel = 0; row_sel < m; row_sel++)
                solution_vect(row_sel + n, 0) = -var.lam(row_sel); // there will be an inversion inside FromVectorToUnknowns()
		}
